...
| kindnet keeps up: no CPU quota, no memory-limit hits, nothing left waiting for a verdict | pass | 2847 new flows judged since kindnet started, 0 dropped |
...
| The settlement preview runs on the cluster | pass | 400 members planned in 18.31 ms by splitx-559b8c7898-4j59d (65 ms round trip) |
...
| The autoscaler is reading real CPU from metrics-server | pass | 0% of the 250m request, target 60% |
...
| No request was lost while every pod was replaced | pass | 3024 requests in 5.8 s: 3024 OK, 0 non-200, 0 failed |
| Traffic settles on the new pods | pass | within 1080 ms of the rollout completing; 2 pod(s) answered during it |
...
| One request can be followed in Loki from the ingress to the pod that served it | pass | X-Request-Id b50384c9fc3de8b9c510182d0c94c21e: ingress-nginx sent it to 10.244.1.44:3000 (0.041 s at the edge), and splitx-6b476cbf47-dlcwm at 10.244.1.44 logged the same ID |
...
|---|---|---|---|
| `splitx-559b8c7898-4j59d` | splitx-worker | 10.244.1.43 | 0 |
| `splitx-559b8c7898-mp7xb` | splitx-worker2 | 10.244.2.27 | 0 |
...
|---|---|---|---|---|---|---|---|
| `splitx-control-plane` | max | 0 of 0 | 63 MiB (78) of 256 MiB | 0 | 0 | 0 | 0 |
| `splitx-worker` | max | 0 of 0 | 59 MiB (78) of 256 MiB | 0 | 1386 | 0 | 0 |
| `splitx-worker2` | max | 0 of 0 | 63 MiB (78) of 256 MiB | 0 | 1461 | 0 | 0 |
...
| `POST /api/settlements/preview` (400 members) | HTTP 200, planned in 18.31 ms |
...
| | |
|---|---|
| Requests during the release | 3024 in 5.8 s |
| HTTP 200 | 3024 |
| Non-200 responses | 0 |
| Connection failures | 0 |
| Pods that answered | 2 |
| Traffic fully on the new pods | 1080 ms after the rollout reported complete |
...
```
splitx-559b8c7898-mp7xb  1513
splitx-559b8c7898-4j59d  1511
```
...
| | |
|---|---|
| X-Request-Id | `b50384c9fc3de8b9c510182d0c94c21e` |
| ingress-nginx access log | status 200, 0.041 s, sent to `10.244.1.44:3000` |
| Application log | `splitx-6b476cbf47-dlcwm` at `10.244.1.44` |
