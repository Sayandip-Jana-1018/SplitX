...
| kindnet keeps up: no CPU quota, no memory-limit hits, nothing left waiting for a verdict | pass | 564 new flows judged since kindnet started, 0 dropped |
...
| The settlement preview runs on the cluster | pass | 400 members planned in 14.57 ms by splitx-7b488cc799-9shvr (49 ms round trip) |
...
| No request was lost while every pod was replaced | pass | 2236 requests in 6.9 s: 2236 OK, 0 non-200, 0 failed |
| Traffic settles on the new pods | pass | within 546 ms of the rollout completing; 3 pod(s) answered during it |
...
| Alerts are routed to email, and every email was accepted | **fail** | no email receiver: .env has no ALERT_SMTP_USERNAME, ALERT_SMTP_PASSWORD and ALERT_EMAIL_TO |
| Grafana serves the committed dashboards, and every data source they name exists | pass | SplitX · Logs (7 panels), SplitX · Service (27 panels); data sources loki, prometheus |
| Every dashboard query runs, and reads metrics that exist | pass | 38 queries on 28 metrics |
| Every application pod is logging to Loki | pass | 2 of 2 pods have lines in the last 15 minutes |
| One request can be followed in Loki from the ingress to the pod that served it | pass | X-Request-Id 4c907eff6dff4317974c2839456a0668: ingress-nginx sent it to 10.244.2.12:3000 (0.037 s at the edge), and splitx-55f7659dcc-99vvc at 10.244.2.12 logged the same ID |
...
|---|---|---|---|
| `splitx-7b488cc799-9shvr` | splitx-worker2 | 10.244.2.9 | 0 |
| `splitx-7b488cc799-x2px7` | splitx-worker | 10.244.1.22 | 0 |
...
|---|---|---|---|---|---|---|---|
| `splitx-control-plane` | max | 0 of 0 | 61 MiB (78) of 256 MiB | 0 | 0 | 0 | 0 |
| `splitx-worker` | max | 0 of 0 | 60 MiB (78) of 256 MiB | 0 | 275 | 0 | 0 |
| `splitx-worker2` | max | 0 of 0 | 64 MiB (78) of 256 MiB | 0 | 289 | 0 | 0 |
...
| `POST /api/settlements/preview` (400 members) | HTTP 200, planned in 14.57 ms |
...
| | |
|---|---|
| Requests during the release | 2236 in 6.9 s |
| HTTP 200 | 2236 |
| Non-200 responses | 0 |
| Connection failures | 0 |
| Pods that answered | 3 |
| Traffic fully on the new pods | 546 ms after the rollout reported complete |
...
```
splitx-7b488cc799-x2px7  910
splitx-7b488cc799-9shvr  903
splitx-55f7659dcc-99vvc  423
```
...
| | |
|---|---|
| X-Request-Id | `4c907eff6dff4317974c2839456a0668` |
| ingress-nginx access log | status 200, 0.037 s, sent to `10.244.2.12:3000` |
| Application log | `splitx-55f7659dcc-99vvc` at `10.244.2.12` |
