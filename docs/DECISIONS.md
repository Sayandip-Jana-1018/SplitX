...
- **The local cluster is not given the production database.** `.env` has a
  `DATABASE_URL` pointing at Neon; using it would have made every rehearsal —
  including the "scale the database to zero" test above — run against the live
  data of the deployed site. The script builds an in-cluster URL instead and
  ignores the one in the file.
