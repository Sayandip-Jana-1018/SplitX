...
| **Kubernetes** | Three-node cluster (Kind locally, EKS for the demo) on the same pinned version; one Kustomize base with local, AWS and load-test overlays; in-cluster Postgres built from the Prisma schema; pods non-root on a read-only filesystem, enforced by the namespace; default-deny networking; config changes and new builds always roll the pods | `npm run k8s:verify` — 24 live checks, including a database outage, a release under traffic and fresh connections from every pod: [docs/evidence/kubernetes.md](docs/evidence/kubernetes.md) |
...
| **Load tests** | k6 2.2.0 in its pinned container: a classroom under production limits, a held overload on two pods, and a staircase that drives the autoscaler; a runner applies the setup each test declares and samples the cluster while it runs | Raw results committed in `docs/evidence/load/`, rendered as [docs/evidence/load-tests.md](docs/evidence/load-tests.md) |
