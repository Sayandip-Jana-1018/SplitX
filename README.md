...
| **Delivery** | A merge to main becomes a release: GitHub Actions builds the image once, scans it (Trivy), signs it keyless (cosign) and publishes it to ghcr.io by commit, then announces it as a GitHub deployment. GitHub’s signed webhook reaches Jenkins in the cluster through a smee.io relay; Jenkins verifies the webhook and the image signature, applies the release commit’s own manifests by digest, checks the rollout through the ingress, and rolls back anything that does not come up | `npm run cd:verify` — 13 live checks: [docs/evidence/delivery.md](docs/evidence/delivery.md), and a Grafana dashboard of the path. A signed release deployed in 177 s; a release built to fail was rolled back with 717 of 717 requests still answered 200 |
| **Supply chain** | Every release is built once, scanned, and signed keyless with cosign — the certificate names the workflow, repository and commit, so there is no signing key to leak. The cluster then refuses, at admission, any image from our registry that this repository’s release workflow on main did not sign (Kyverno), and a daily job re-scans whatever is actually running, because a scan at build time ages | `npm run cd:verify`: the signed release is admitted, a tag we never published is refused, and the same signed image judged against another workflow is refused — the policy checks whose signature it is |
...
8. Run it on Kubernetes: `npm run k8s:up` creates the three-node Kind cluster, installs the pinned platform charts, loads the image into the nodes, builds the Secrets from `.env` and applies `k8s/overlays/local` and the dashboards. The app is then on <http://localhost/>, and Grafana on <http://grafana.localhost/> (user `admin`, password `GF_ADMIN_PASSWORD`), with the SplitX Service and Logs dashboards.
   ...
   - `npm run k8s:pause` puts the cluster to sleep without deleting anything, and `npm run k8s:resume` wakes it and waits until the app answers. A paused cluster stays paused across Docker and Windows restarts; otherwise it starts whenever Docker does. Its nodes may use 2 + 4 + 4 CPUs of the laptop, never all of it (D-062).
