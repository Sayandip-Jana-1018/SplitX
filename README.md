...
| **Supply chain** | Every release is built once, scanned, and signed keyless with cosign — the certificate names the workflow, repository and commit, so there is no signing key to leak. The cluster then refuses, at admission, any image from our registry that this repository’s release workflow on main did not sign (Kyverno) | `npm run cd:verify`: the signed release is admitted, a tag we never published is refused, and the same signed image judged against another workflow is refused — the policy checks whose signature it is |
